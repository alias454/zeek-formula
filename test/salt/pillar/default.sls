  ...
  lookup:
    ...
    {% if grains['os_family'] == 'RedHat' %}
      ...
      local_package:                                # Can be multiple packages like zeek, zeekctl, zeekccoli etc.
        - pack_id: 'zeek-full'
          package: 'Zeek-2.6-195-Linux-x86_64'       # Custom package to be deployed
    ...
    {% if grains['osfinger'] == 'Ubuntu-16.04' %} #Ubuntu 16 throws an error for now disabling this until fixed
      use_ZeekPKG: 'False'                    # Use zeek-pkg to manage plugins (requird for plugins such as af_packet etc)
    {% else %}
      use_ZeekPKG: 'False'                    # Use zeek-pkg to manage plugins (requird for plugins such as af_packet etc)
    {% endif %}
    {% if grains['os_family'] == 'RedHat' %}
      python_pip_pkg: 'python36-pip'         # Can be pip or pip3 but only gets installed if use_ZeekPKG == True
    {% elif grains['os_family'] == 'Debian' %}
      python_pip_pkg: 'python3-pip'         # Can be pip or pip3 but only gets installed if use_ZeekPKG == True
    {% endif %}
      python_pip_cmd: 'pip3'                 # Otherwise, the pip install is skipped and zeek-pkg is not configured
    ...
    {% elif grains['os_family'] == 'Debian' %}
      ...
      LogDir: '/var/log/zeek'                 # Default location of log directory on Debian
      SpoolDir: '/var/spool/zeek'             # Default location of spool directory on Debian
      CfgDir: '/etc/zeek'                     # Default config directory location for Debian
      ShareDir: '/usr/share/zeek'             # Location of shared resources (site local)
    {% endif %}
